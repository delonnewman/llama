...
sub alias ($self, %aliases) {
  for my $original (keys %aliases) {
    ...
    Carp::croak "aliases should be fully qualified, " . "got '$alias' instead"
      unless $alias =~ /::/;
...
sub symbol_names ($self, $type = undef) {
  ...
  Carp::carp "symbol table is empty this could mean that "
    . "the package isn't loaded, try calling the 'load' method"
    unless %table;
  ...
  @names
    = grep { ref(\$table{$_}) eq 'GLOB' && defined(*{ $table{$_} }{$type}) }
    @names;
