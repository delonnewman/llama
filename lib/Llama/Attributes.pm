...
sub import ($class) {
  ...
  my $order = 0;
  $pkg->add_sub(
    'has' => sub ($name, $options = $EMPTY_HASH) {
      $caller->class->add_attribute($name, { order => $order++, %$options });
    }
  );

  $pkg->add_sub(
    'has_many' => sub ($name, $options = $EMPTY_HASH) {
      Llama::Package->named($options->{class})->maybe_load
        if $options->{class};
      $caller->class->add_attribute(
        $name,
        {
          optional    => 1,
          associated  => 1,
      ...
      $caller->class->add_method(
        "has_$name" => sub ($self) { $self->has_some($name) });
    },
  );
}
